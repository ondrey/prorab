<template>
  
    <v-dialog v-model="dialog" fullscreen hide-overlay transition="dialog-bottom-transition">
      
      <template v-slot:activator="{ on }">
          <v-list-item v-on="on">
            <v-list-item-icon>
              <v-icon>mdi-account-box</v-icon>
            </v-list-item-icon>
            <v-list-item-title>Сотрудники</v-list-item-title>
          </v-list-item>
      </template>

      <v-card>
        
        <v-toolbar dark color="primary">
        <v-btn icon dark @click="dialog = false">
        <v-icon>mdi-close</v-icon>
        </v-btn>
        <v-toolbar-title>Сотрудники</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-toolbar-items>
        
        </v-toolbar-items>
        </v-toolbar>
      
        <v-card style="margin:10px; background-color: greenyellow; padding:8px">
            <v-text-field
            v-model="familiya"
            label="Фамилия"
            ></v-text-field>
            <v-text-field
            v-model="imya"
            label="Имя"
            ></v-text-field>        

            <v-btn width="100%" color="success" @click="add_empl_click">Добавить</v-btn>

        </v-card>
        


      <v-list>
        <v-list-item v-for="k in listEmpl" :key="k.id">
          <v-list-item-content>
            {{k.familiya}} 
            <v-list-item-subtitle>
            {{k.name}}
            </v-list-item-subtitle>
          </v-list-item-content>

        <v-list-item-action>
                    
          <v-btn icon @click="del_empl(k.id)">
            <v-icon color="red">mdi-delete-alert-outline</v-icon>
          </v-btn>

        </v-list-item-action>

        </v-list-item>

      </v-list>

      </v-card>

    </v-dialog>

</template>

<script>

export default {
    data(){
        return {
            dialog: false,
            pickertime: "17:00",
            listEmpl: [],
            familiya: "",
            imya: ""
        }
    },
    methods:{      
      add_empl_click(){        
        this.$root.add_record('Emploeys',[
          {familiya: this.familiya, name: this.imya},
        ]) 
        this.refresh()       
      },
      set_empl_list(event){
        const cursor = event.target.result
        if(cursor) {
          
          this.listEmpl.push({
            id:cursor.key, 
            familiya:cursor.value.familiya, 
            name:cursor.value.name})   

          cursor.continue()
        }  
      },
      del_empl(idnum){        
        if (confirm("Удалить выбранную запись?")){
          let id_empl = idnum
          this.$root.connectDB(db => {
            let tx = db.transaction(['Emploeys'], 'readwrite')
            let empl = tx.objectStore('Emploeys')
            let res = empl.delete(id_empl)
            res.onsuccess = this.refresh
          })
        }
        
      },
      refresh(){
        this.$root.connectDB(db => {
          this.listEmpl = []
          let tx = db.transaction(['Emploeys'], 'readwrite')
          let empl = tx.objectStore('Emploeys')
          empl.openCursor().onsuccess = this.set_empl_list       
        })
      }
    },
    created(){
      this.refresh()   
    }
}
</script>

<style>

</style>